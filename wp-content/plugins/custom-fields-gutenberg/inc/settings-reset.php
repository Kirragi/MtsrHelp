	...
	if ($screen_id === 'settings_page_g7g-cfg') {
		...
		if (!g7g_cfg_check_date_expired() && !g7g_cfg_dismiss_notice_check()) {
			...
			<div class="notice notice-success">
				<p>
					<strong><?php esc_html_e('Go Pro!', 'custom-fields-gutenberg'); ?></strong> 
					...
					<?php esc_html_e('Apply code', 'custom-fields-gutenberg'); ?> <code>PLANET24</code> <?php esc_html_e('at checkout. Sale ends 5/25/24.', 'custom-fields-gutenberg'); ?> 
...
function g7g_cfg_dismiss_notice_link() {
	...
	echo '<a class="g7g-cfg-dismiss-notice" href="'. esc_url($href) .'">'. esc_html($label) .'</a>';
	
}

function g7g_cfg_check_date_expired() {
	
	$expires = apply_filters('g7g_cfg_check_date_expired', '2024-05-25');
